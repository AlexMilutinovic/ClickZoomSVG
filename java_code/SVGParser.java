...
//        Util.debug("Parsing complete in " + (System.currentTimeMillis() - start) + " millis.");
            ...
            result.setCities(handler.getCities());
            return result;
        } catch (Exception e) {
    ...
    private static NumberParse parseNumbers(String s) {
        //Log.d(TAG, "parseNumbers: " + "Parsing numbers from: '" + s + "'");
        int n = s.length();
        int p = 0;
            ...
            switch (c) {
                ...
                case ')': {
                    ...
                    if (str.trim().length() > 0) {
                        //Log.d(TAG, "parseNumbers: " + "  Last: " + str);
                ...
                case '-': {
                    ...
                    if (str.trim().length() > 0) {
                        //Log.d(TAG, "parseNumbers: " + "  Next: " + str);
                        //Util.debug("  Next: " + str);
                        Float f = Float.parseFloat(str);
                        numbers.add(f);
                        if (c == '-') {
...
//            Log.d(TAG, "Float parsing '" + name + "=" + v + "'");
    ...
    private static class SVGHandler extends DefaultHandler {
        ...
        private Map<String, FintemCity> cities;
        private boolean linksStarted = false;
        ...
        private void parseElementAttributes(Attributes attributes) throws NullPointerException{
            if (attributes == null){
                return;
            }
            int n = attributes.getLength();
            FintemCity city = new FintemCity();
            for (int i = 0; i < n; i++) {
                String att = attributes.getLocalName(i);
                if (att != null) {
                    ...
                    switch (att){
                        case "x":
                            city.setX((int)Float.parseFloat(value.replace("px", "")));
                            break;
                        case "y":
                            city.setY((int)Float.parseFloat(value.replace("px", "")));
                            break;
                        case "width":
                            city.setW((int)Float.parseFloat(value.replace("px", "")));
                            break;
                        case "height":
                            city.setH((int)Float.parseFloat(value.replace("px", "")));
                            break;
                        case "onclick":
                            String[] splittedLink = attributes.getValue(att).split("/");
                            int index = splittedLink.length - 2;
                            city.setTitle(splittedLink[index].replace("-", "_"));
                            break;
                    }
                }
                if (city.isValid()){
                    cities.put(city.getTitle(), city);
                }
            }
        }

        public Map<String, FintemCity> getCities(){
            return cities;
        }

        @Override
        public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
            String links = getStringAttr("id", atts);
            this.drawCharacters = false;
            if (localName.equals("g") && links != null && links.equals("Links")){
                linksStarted = true;
            }
            if (linksStarted && localName.equals("rect")){
                parseElementAttributes(atts);
            }
            // Reset paint opacity
            paint.setAlpha(255);
            ...
            if (localName.equals("svg")) {
                ...
                if (fontSize != null) {
                    ...
                    Properties props = new Properties(atts);
                    Integer color = props.getHex("fill");
                    if (color != null) {
            ...
            if (this.drawCharacters) {
                if (length == 1 && ch[0] == '\n') {
                    ...
                    if (this.textX != null && this.textY != null) {
                        canvas.drawText(text, this.textX, this.textY, paint);
                    } else {
                        canvas.setMatrix(font_matrix);
                        canvas.drawText(text, 0, 0, paint);
                    }
                    Float delta = paint.measureText(text);
